...
# Script para ver logs en tiempo real (Entorno Local)
...
show_menu() {
    clear
    echo -e "${CYAN}╔════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║       📊 VISOR DE LOGS - LOCAL            ║${NC}"
    echo -e "${CYAN}╚════════════════════════════════════════════╝${NC}"
    echo ""
...
view_laravel_file_logs() {
    ...
    docker exec -it $CONTAINER_APP tail -f /var/www/storage/logs/laravel.log
}
...
view_laravel_errors() {
    ...
    docker exec -it $CONTAINER_APP tail -f /var/www/storage/logs/laravel.log | grep -i "error\|exception\|fatal"
}
...
view_laravel_last() {
    ...
    docker exec -it $CONTAINER_APP tail -50 /var/www/storage/logs/laravel.log
...
search_in_logs() {
    ...
    docker exec -it $CONTAINER_APP grep -i "$search_term" /var/www/storage/logs/laravel.log | tail -50
