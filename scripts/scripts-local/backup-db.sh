...
# Script para backup y restauración de MySQL (Entorno Local)
...
show_menu() {
    clear
    echo -e "${CYAN}╔════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║  💾 BACKUP DE BASE DE DATOS - LOCAL       ║${NC}"
    echo -e "${CYAN}╚════════════════════════════════════════════╝${NC}"
    echo ""
...
create_backup() {
    ...
    docker exec $CONTAINER_MYSQL mysqldump -u $DB_USER -p"$DB_PASSWORD" $DB_NAME > $BACKUP_PATH 2>/dev/null
...
restore_backup() {
    ...
    # Restaurar
    docker exec -i $CONTAINER_MYSQL mysql -u $DB_USER -p"$DB_PASSWORD" $DB_NAME < $temp_file 2>/dev/null
    ...
    if [ $? -eq 0 ]; then
        ...
        docker exec laravel-app-1 php artisan config:clear 2>/dev/null
        docker exec laravel-app-1 php artisan cache:clear 2>/dev/null
