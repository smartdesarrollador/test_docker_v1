...
# Script para monitorear estado de contenedores Docker
...
get_container_status() {
    ...
    elif sudo docker ps -a --filter "name=$container" --format "{{.Status}}" | grep -q "Exited"; then
...
get_container_uptime() {
    local container=$1
    sudo docker ps --filter "name=$container" --format "{{.Status}}" 2>/dev/null | sed 's/Up //'
}

# Función para obtener health del contenedor
get_container_health() {
    local container=$1
    local health=$(sudo docker inspect --format='{{.State.Health.Status}}' $container 2>/dev/null)
...
show_dashboard() {
    clear
    echo -e "${CYAN}╔════════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║              📊 DASHBOARD DE CONTENEDORES                         ║${NC}"
    echo -e "${CYAN}╚════════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    echo -e "   IP:          $(sudo docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $CONTAINER_APP 2>/dev/null || echo 'N/A')"
    echo -e "   Puerto:      9000"
    echo ""
    ...
    echo -e "   IP:          $(sudo docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $CONTAINER_NGINX 2>/dev/null || echo 'N/A')"
    echo -e "   Puerto:      80"
    echo ""
    ...
    echo -e "   IP:          $(sudo docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $CONTAINER_MYSQL 2>/dev/null || echo 'N/A')"
    echo -e "   Puerto:      3306"
    ...
    echo -e "   IP:          $(sudo docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $CONTAINER_REDIS 2>/dev/null || echo 'N/A')"
    echo -e "   Puerto:      6379"
    echo ""
    ...
    running=$(sudo docker ps | grep -c "api-laravel-1" || echo 0)
    total=$(sudo docker ps -a | grep -c "api-laravel-1" || echo 0)
    
    echo -e "   Contenedores corriendo: ${GREEN}$running${NC}/$total"
    echo -e "   Red:                     ${CYAN}global${NC}"
    echo -e "   URL API:                 ${CYAN}https://api.smartdigitaltec.com${NC}"
    echo ""
...
show_resources() {
    ...
    sudo docker stats --format "table {{.Container}}\t{{.CPUPerc}}\t{{.MemUsage}}\t{{.MemPerc}}\t{{.NetIO}}\t{{.BlockIO}}" \
...
show_container_details() {
    ...
    sudo docker inspect $selected --format='
...
show_networks() {
    ...
    echo ""
    echo -e "${YELLOW}Detalles de la red 'global':${NC}"
    echo ""
    
    sudo docker network inspect global --format='
Nombre:       {{.Name}}
ID:           {{.Id}}
Driver:       {{.Driver}}
...
show_volumes() {
    ...
    sudo docker volume inspect api-laravel-1_mysql_data --format='
Nombre:       {{.Name}}
Driver:       {{.Driver}}
...
' 2>/dev/null
    ...
    sudo du -sh /var/lib/docker/volumes/api-laravel-1_mysql_data 2>/dev/null || echo "N/A"
...
show_images() {
    ...
    sudo docker images --format "table {{.Repository}}\t{{.Tag}}\t{{.Size}}\t{{.CreatedSince}}"
